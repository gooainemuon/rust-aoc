...
fn part1_00(a: &str) {
    println!("코랩에서 파이썬으로 짰던 코드를 rust로 직역한 것");
    println!("문자열 길이를 만큼 문자열을 순회하며 비교");
...
fn part1_01(a: &str) {
    println!("ai가 함수형 프로그래밍 스타일로 교정한것");
    println!("러스트 문자열은 UTF-8이라 단순 인덱싱불가.");
    println!("문자열의 유니코드 스칼라값을 순회하는 반복자를 반환하는 chars() 메소드 사용");
    println!("참고: UTF-8의 바이트 단위는 bytes() 메소드 사용");
...
fn main() {
    ...
    println!("피드백: 변수명을 a, aa로 쓰지말고 기능과 목적에 맞게 지었으면 좋겠다.");
}
